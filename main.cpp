...
struct canvas
{
	canvas(int canvas_area, color population_color, int canvas_width, int canvas_height) : canvas_final_draw(canvas_area)
	{
		...
		for(int i = 0; i < canvas_height; i++)
		{
			...
			{
				canvas_final_draw[counter].pixel_init(i,j);
				counter++;
			}
 ...
 *
	//Filling in x and y.
	void fill_vec2_canvas_End(int width, int height)
	{
	...
	the problem is that c++ is treating my constructor as a function declaration :(
...
struct canvas
{
	canvas(int canvas_area, color population_color) : cv(canvas_area), canvas_final_draw(canvas_area)
	{
		for (unsigned int i = 0; i < canvas_area; i++)
		{
			cv[i] = (population_color);
		}
	}


	std::vector<vec2> canvas_final_draw;
	std::vector<color> cv;
...
int main()
{
	...
	canvas canvas_object(total_index, red);
	...
	for(unsigned int i = 0; i < total_index; i++)
	{
		file << canvas_object.cv[i].r << " " << canvas_object.cv[i].g << " " << canvas_object.cv[i].b << " ";
	}
...
main.cpp:88:21: error: expected identifier before numeric constant
   88 |         vec3 camera(0,0,0); //this is the origin
      |                     ^
main.cpp:88:21: error: expected ‘,’ or ‘...’ before numeric constant
